...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/aslabotHW5"
## Provided:
...
class UpdatePriority(FlaskForm):
    newPriority = IntegerField("What is the new priority of this item? ", validators=[Required()])
    update = SubmitField("Update")
...
def update(item):
    ...
    form = UpdatePriority()
    if request.method == "POST":
        itemToUpdate = TodoItem.query.filter_by(id=item).first()
        itemToUpdate.priority = request.form['newPriority']
        db.session.commit()
        flash('Updated priority of ' + itemToUpdate.description)
        return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    #if request.method == 'POST':
    #    pass
    listToDelete = TodoList.query.filter_by(id=lst).first()
    db.session.delete(listToDelete)
    db.session.commit()
    flash('Deleted list ' + listToDelete.title)
    return redirect(url_for('all_lists'))
